"""
stress_engine.py - Motor de estrés (VIX invertido)
Calcula el score de estrés usando el VIX invertido,
...
class StressEngine:
    def __init__(self, config_path='config/config.yaml'):
        ...
        cfg = self.config.get('stress_engine', {})
        self.tickers = cfg.get('tickers', ['^VIX'])
        ...
        self.invert_signal = cfg.get('invert_signal', True)
    ...
    def calcular_todo(self, df):
        ...
        # El VIX lo tomamos directamente como nivel (no retorno) porque la documentación dice "nivel"
        vix = df[self.tickers[0]]
        if self.invert_signal:
            raw = -vix  # Invertimos para que score alto = estrés bajo
        else:
            raw = vix
        ...
        resultados = pd.DataFrame(index=df.index, columns=['score_stress'])
        resultados['score_stress'] = score
