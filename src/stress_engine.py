"""
stress_engine.py - Motor de estrés sistémico (versión simplificada)
Calcula el score de estrés usando el retorno diario del VIX (^VIX) con signo invertido,
...
class StressEngine:
    def __init__(self, config_path='config/config.yaml'):
        ...
        # Cargar configuración específica del motor de estrés
        cfg = full_config.get('stress_engine', {})
        ...
        self.invert_signal = cfg.get('invert_signal', True)  # True para que subida de VIX = estrés = score negativo

    def calcular_todo(self, df):
        """
        df: DataFrame con columna de precio (^VIX)
        ...
        resultados['score_stress'] = normalized
