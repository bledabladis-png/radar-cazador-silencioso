...
def generate_daily_report(macro_score, macro_regime, macro_conf, liquidity_score, liquidity_regime, liq_conf,
    ...
    if breadth_values:
        ...
        lines.append("|---------|-------|\n")
        lines.append(f"| % sobre EMA20 | {breadth_values.get('% sobre EMA20', 0):.2%} |\n")
        lines.append(f"| % sobre EMA50 | {breadth_values.get('% sobre EMA50', 0):.2%} |\n")
        lines.append(f"| % sobre EMA200 | {breadth_values.get('% sobre EMA200', 0):.2%} |\n")
        lines.append(f"| New Highs sectoriales (0/11) |\n")
        lines.append(f"| New Lows sectoriales (1/11) |\n")
        lines.append("\n")
    ...
    if slpm_v12_data:
        ...
        if inputs:
            ...
            lines.append(f"- **Scores oficiales:** T={inputs.get('tactical', 0):+.2f} | S={inputs.get('structural', 0):+.2f} | LIS={inputs.get('lis', 0):+.2f} | Eff Breadth={eff_breadth:.2f} | Persist={pers_str}\n")
        ...
        if breadth:
            ...
            lines.append(f"- **Effective Breadth (Health {comp:.0f}% x Cobertura {coverage:.0f}%):** {effective:.0f}%\n")
            lines.append(f"  - N analizado: {n}/{total}\n")
            lines.append(f"  - *Nota: La confianza ajustada reduce la senhal por baja cobertura. La calidad observada (Health Composite) es independiente de la cobertura.*\n")
