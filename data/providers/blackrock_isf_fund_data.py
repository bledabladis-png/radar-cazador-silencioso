...
def download_fund_file():
    ...
    if use_cache:
        print('  Usando caché para ISF.L')
        return True

    print('  Descargando ISF.L desde BlackRock...')
    headers = {
        'User-Agent': 'Mozilla/5.0',
        'Referer': 'https://www.blackrock.com/es/profesionales/productos/251795/'
    }
    try:
        r = requests.get(URL_ISF, headers=headers, timeout=60)
        r.raise_for_status()
        ...
        print(f'  Error descargando ISF.L: {e}')
...
def get_blackrock_isf_primary_flow(force_download: bool = False) -> pd.DataFrame:
    """Descarga/procesa el histórico de ISF.L y devuelve la última fila con flujo primario."""
    if force_download:
        download_fund_file()
    else:
