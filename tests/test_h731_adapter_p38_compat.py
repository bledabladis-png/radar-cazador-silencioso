...
Regresion del hallazgo H-73.1: los records producidos por
catalog_to_p38_targets deben ser semanticamente compatibles con
compute_contractual_coverage (operational_mapping_status == VERIFIED).

El bug original mapeaba period_state.weight_status (RESOLVED_OBSERVED)
directamente a PositionRecord.operational_mapping_status, lo que
hacia que P38 filtrara los records como no-VERIFIED y devolviera
UNAVAILABLE.

Este test es exactamente el que faltaba: verifica no solo la
estructura del PositionRecord, sino su COMPATIBILIDAD SEMANTICA
con el consumidor (compute_contractual_coverage).
"""
...
def test_h731a_records_del_adapter_son_verified():
...
def test_h731b_weight_status_no_contamina_operational():
...
def test_h731c_p38_sobre_records_del_adapter():
    """Test que faltaba: los records del adapter deben pasar por
    compute_contractual_coverage y producir VALID (no UNAVAILABLE)."""
    u = _make_universe([("AAPL", "FIGI_A"), ("MSFT", "FIGI_M")])
    st = ps.build_period_state(u)
    keys = set(u.declared_keys)
...
def test_h731d_regresion_bug_original():
...
def test_h731e_state_no_resolved_excluido():
