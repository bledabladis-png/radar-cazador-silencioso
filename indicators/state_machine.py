...
def classify_leadership_state(tactical_score, structural_score, leader_breadth, persistence, coverage=1.0):
...
def validate_state(state, tactical_score, structural_score, leader_breadth, persistence):
    errors = []
    t = THRESHOLDS
    ...
    if state == 'TACTICAL_CORRECTION':
        ...
        if not (leader_breadth > 0.35):
            errors.append(f"TACTICAL_CORRECTION requiere Breadth > 0.35, pero es {leader_breadth:.2f}")
