...
def compute_stock_metrics_for_index(df_stocks, index_name, stock_list, df_index_data=None):
    ...
    for ticker in stock_list:
        ...
        spring = detect_spring(df_stocks, ticker).iloc[-1] if len(df_stocks) > 20 else 0
        sos = detect_sos(df_stocks, ticker).iloc[-1] if len(df_stocks) > 20 else 0
