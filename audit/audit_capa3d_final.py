...
print("=" * 70)
print("CAPA 3D FINAL: SLPM v1.2 con líderes reales (solo sectores con datos)")
print("=" * 70)
...
fechas = df_market.index[::10]  # cada 10 días
resultados = []
omitidas = 0
...
for i, fecha in enumerate(fechas):
    try:
        ...
        # Solo procesar si hay líderes reales para este sector
        ...
        for ticker in top_holdings:
            try:
                ...
                flow_raw = ret_stk * close_stk * vol_stk
                flow_z = (flow_raw - flow_raw.rolling(60).median()) / (flow_raw.rolling(60).mad() + 1e-9)
                flow_signal = flow_z.ewm(span=5).mean().iloc[-1]
                leader_metrics.append({"ticker": ticker, "rs": rs_stk.iloc[-1], "rs_momentum": rs_mom if pd.notna(rs_mom) else 0.0, "flow_z": flow_signal if pd.notna(flow_signal) else 0.0, "wyckoff_phase": "RANGE"})
            except: pass
        ...
        tactical_scores = {s: compute_tactical_score(mkt, s) for s in sectores}
        structural_scores = {s: compute_structural_score(mkt, s) for s in sectores}
        sector_persistence = {}
        for s in sectores:
            try:
        ...
        slpm = evaluate_slpm_v12(mkt, sector_results, leader_metrics, 0.0, tactical_scores=tactical_scores, structural_scores=structural_scores, sector_persistence=sector_persistence)
        if slpm:
            lb = slpm.get("leader_breadth_v2", {})
            li = slpm.get("leader_integrity", {})
            fd = slpm.get("flow_divergence_v2", {})
            ins = slpm.get("input_scores", {})
            resultados.append({"fecha": fecha, "sector": slpm.get("sector",""), "effective_breadth": lb.get("effective_composite", 0.5), "lis": li.get("lis", 0.0), "flow_divergence_composite": fd.get("composite", 0.0), "tactical_score": ins.get("tactical", 0.0), "structural_score": ins.get("structural", 0.0), "persistence": ins.get("persistence", 0.5)})

        if (i+1) % 20 == 0:
            print(f"  {i+1}/{len(fechas)} fechas (válidas: {len(resultados)})")
    except: pass

print(f"\nSemanas válidas: {len(resultados)} (omitidas por falta de líderes: {omitidas})")
