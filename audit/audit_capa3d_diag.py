import pandas as pd, numpy as np, os, sys, warnings, traceback
...
print("=" * 70)
print("CAPA 3D DIAGNÓSTICO COMPLETO")
print("=" * 70)
...
print(f"  market_data: {len(df_market)} filas, {df_market.index[0].date()} a {df_market.index[-1].date()}")
print(f"  stocks: {len(df_stocks)} filas, {df_stocks.index[0].date()} a {df_stocks.index[-1].date()}")

# Alinear fechas
common_start = max(df_market.index[0], df_stocks.index[0])
common_end = min(df_market.index[-1], df_stocks.index[-1])
df_market = df_market.loc[common_start:common_end]
df_stocks = df_stocks.loc[common_start:common_end]
print(f"  Rango común: {common_start.date()} a {common_end.date()} ({len(df_market)} días)")

# 2. Generar fechas de muestreo
fechas = df_market.index[::10]
print(f"\n[2] Procesando {len(fechas)} fechas (cada 10 días)")

resultados = []
stats = {"total": len(fechas), "sin_datos": 0, "sin_lideres": 0, "slpm_ok": 0, "slpm_none": 0, "error": 0}
...
for i, fecha in enumerate(fechas):
    try:
        ...
        top_etf = ranking[0][0]
        top_name = SECTOR_NAMES.get(top_etf, top_etf)
        ...
        if len(leader_metrics) < 3:
            stats["sin_lideres"] += 1
            if i < 5:  # Mostrar primeras 5 omisiones
                print(f"  [{i}] {fecha.date()} | Líder: {top_name} ({top_etf}) | Holdings: {len(top_holdings)} | Líderes encontrados: {len(leader_metrics)} -> OMITIDA")
            continue
        ...
        if slpm:
            ...
            stats["slpm_ok"] += 1
            if i < 5:
                print(f"  [{i}] {fecha.date()} | {top_name} | Estado: {slpm.get('state','?')} | Breadth: {lb.get('effective_composite',0):.2f}")
        else:
            stats["slpm_none"] += 1
            if i < 5:
                print(f"  [{i}] {fecha.date()} | SLPM retornó None")

        if (i + 1) % 30 == 0:
            print(f"  Progreso: {i+1}/{len(fechas)} | Válidas: {len(resultados)} | Sin líderes: {stats['sin_lideres']}")

    except Exception as e:
        stats["error"] += 1
        if stats["error"] <= 5:
            print(f"  ERROR [{i}] {fecha.date()}: {e}")
            traceback.print_exc()

# 3. Resumen final
print(f"\n[3] RESUMEN:")
print(f"  Total fechas: {stats['total']}")
print(f"  Sin datos suficientes: {stats['sin_datos']}")
print(f"  Sin líderes (<3): {stats['sin_lideres']}")
print(f"  SLPM OK: {stats['slpm_ok']}")
print(f"  SLPM retornó None: {stats['slpm_none']}")
print(f"  Errores: {stats['error']}")
print(f"  Registros válidos: {len(resultados)}")

if len(resultados) < 20:
    print("\nERROR: Muy pocos datos para correlaciones fiables.")
    sys.exit(1)

# 4. Matriz de correlación
print("\n[4] Calculando matriz Spearman...")
