...
Version anterior: 1.0 (commit e6f550a; preservada como
                  INSTITUTIONAL_ACCUMULATION_NIPC_ESPECIFICACION_v1.0.md)
Estado: ESPECIFICACION. Sin codigo. Pendiente de dictamen final del
        auditor para autorizar Gate-NIPC.2 (implementacion).
Naturaleza: no normativo. Si hay conflicto con el Prompt Maestro, gana
            el Prompt Maestro.
Referencia: prompt v6.35, HEAD local 6effe85.
...
#### Politica congelada (C4, dictamen Q-ESP-8)
...
Si una resolucion OpenFIGI no puede demostrarse valida para el
`report_period` (2025-12-31 o 2026-03-31):

    mapping_status = TEMPORAL_UNVERIFIED

Y por tanto:

    -> NO entra en el operational_universe
    -> solo diagnostico

Regla adicional: `mapping_timestamp_utc` se registra en cada resolucion.
Si es posterior al `report_period`, la resolucion cae por defecto en
TEMPORAL_UNVERIFIED salvo evidencia documental de validez historica.

#### Pendiente de diseno en Gate-NIPC.2

  - Como obtener evidencia historica de OpenFIGI (si fuera posible).
...
Introducida por dictamen Gate-NIPC.1 (Q-ESP-2), correccion obligatoria
C1.
...
es lo que el sistema mantiene para poder emparejar Q4 y Q1.

#### Capa de derivacion obligatoria

    (report_period, observed_CUSIP)
            v
    canonical_security

Esta capa es OBLIGATORIA. Sin ella:

    CUSIP_A en Q4 2025
    CUSIP_B en Q1 2026 (misma security tras corporate action)
        -> se interpretarian como Exit(A) + New(B)
        -> NIPC incorrecto

Con la capa:

    CUSIP_A en Q4 2025 -> canonical_security_X
    CUSIP_B en Q1 2026 -> canonical_security_X
        -> BOTH (misma security)
        -> delta calculable
...
Por orden de autoridad:
  ...
  1. Internal verified mapping (crosswalk curado + etf_holdings).
     Si el ticker canonico es estable, canonical_security = ticker.
  2. OpenFIGI shareClassFIGI o figi (si el servicio lo devuelve y el
     mapping no es TEMPORAL_UNVERIFIED).
  3. Fallback: el CUSIP observado del periodo con prefijo, SOLO si no
     hay evidencia de cambio. Nunca se asume identidad entre CUSIPs
     distintos sin evidencia.
  4. UNRESOLVED: observed_security_identifier sin canonical_security.

#### Representacion propuesta

    canonical_security_id = string canonico:
        figi:<shareClassFIGI>      -- si OpenFIGI verificada
        ticker:<normalized_ticker> -- si internal + ticker estable
        cusip:<CUSIP>              -- fallback (misma security, sin cambio)
        unresolved:<CUSIP>         -- sin identidad estable

#### Casos de corporate action

Cuando el CUSIP cambia entre periodos por corporate action:

    -> se requiere tabla de equivalencia temporal.
    -> la tabla vive en data/mappings/cusip_equivalence.csv (nueva).
...
Estados de la respuesta:
    ...
    RATE_LIMIT             -- 429
    TEMPORAL_UNVERIFIED    -- ver 3.6 y C4
...
### 7.6. TEMPORAL_UNVERIFIED (C4)

Politica congelada por dictamen Q-ESP-8:
...
Aplicable a todas las resoluciones OpenFIGI mientras no exista
evidencia documental de validez historica (mecanismo a disenar en
Gate-NIPC.2).
...
### 15.1. Deuda del ciclo NIPC
  ...
  - Deuda: temporal_validity de OpenFIGI no verificada.
    Estado: PENDIENTE (politica ya fijada en 3.6, diseno en Gate-NIPC.2).
...
Fin de la especificacion v1.1. Fecha: 2026-09-19. HEAD local al redactar:
6effe85. Estado: pendiente de dictamen del auditor para autorizar
Gate-NIPC.2. La v1.0 se preserva como INSTITUTIONAL_ACCUMULATION_NIPC_
ESPECIFICACION_v1.0.md.
