...
Versiones anteriores: 1.2 (commit 5eeb022; preservada como
...
Estado: ESPECIFICACION. C1 + C4 aplicadas (dictamen v1.1, 2026-09-19).
        Anadida seccion 3.15 filer_continuity (dictamen probe 2026-09-19).
        Gate-NIPC.2 BLOQUEADO: coverage + filer continuity pendientes.
Naturaleza: no normativo. Si hay conflicto con el Prompt Maestro, gana
            el Prompt Maestro.
Referencia: prompt v6.35, HEAD local 0d4a425.
...
Descripcion: la continuidad del FILER CIK entre periodos es una
dimension INDEPENDIENTE de la cobertura de identidad de securities.
Introducida por dictamen probe end-to-end (2026-09-19, Q-PROBE-1 a
Q-PROBE-5).

#### Motivacion

El match key C2 (spec 4.7) es:

    (filing_manager_cik, canonical_security, discretion_type)

Este key mide continuidad del FILER CIK, NO continuidad economica del
conjunto reportado. Un mismo grupo economico puede reorganizar sus
filings entre trimestres (por ejemplo, la entidad padre pasa a 13F-NT
y las filiales presentan 13F-HR). Los CUSIPs permanecen identicos,
pero el match key C2 produce:

    EXIT para el CIK antiguo (presente Q4, ausente Q1)
    NEW para los CIKs nuevos (ausentes Q4, presentes Q1)

Esto es CORRECTO segun contrato. No es un bug.

#### Evidencia empirica (caso Vanguard, Q4 2025 -> Q1 2026)

  Q4 2025:
    CIK 0000102909 (VANGUARD GROUP INC)  13F-HR con holdings.
  Q1 2026:
    CIK 0000102909 (VANGUARD GROUP INC)  13F-NT (notice).
    CIK 0002100119 (VANGUARD CAPITAL MGMT LLC)  13F-HR + 2x13F-HR/A.
    CIK 0002100121 (VANGUARD PORTFOLIO MGMT LLC)  13F-HR + 13F-HR/A.

Los CUSIPs (NVDA, AAPL, AMZN, etc.) son identicos entre periodos.

Impacto cuantitativo (probe ALL scope):
  nipc_sole  = -41.16B
  nipc_dfnd  = +41.28B
  neto       = +52M sobre ~82B brutos (cancelacion por reorg).

#### Reglas congeladas

  1. El match key C2 NO se modifica. Permanece vigente.
  2. Filer continuity es dimension SEPARADA de coverage.
     No se combina en un unico "factor de descuento".
  3. La inferencia economica por parent/child (economic_owner_cik)
     SIGUE PROHIBIDA. Ver FORBIDDEN_TERMS en relationships.py.
  4. Reconciliacion NT <-> HR NO AUTORIZADA todavia.
  5. OpenFIGI masivo NO se inicia antes de cuantificar filer
     continuity (dictamen probe seccion 11, orden de trabajo).

#### Estados observables (mini-probe Q-PROBE-5)

Sin inferencia economica, solo observaciones:

    CONTINUOUS_FILER             CIK presente en ambos periodos con
                                 mismo tipo de filing (HR/HR).

    FILER_DISCONTINUITY          CIK presente en Q4 y ausente en Q1
                                 (o viceversa) sin relacion NT-HR
                                 observable.

    NT_TO_HR_RELATION_OBSERVED   CIK presenta 13F-NT en un periodo
                                 y OTRO CIK presenta 13F-HR que la
                                 evidencia SEC vincula (OTHERMANAGER2).

    HR_TO_HR_WITHOUT_RELATION    Dos CIKs distintos presentan 13F-HR
                                 sin relacion NT-HR observable.

    UNRESOLVED                   No determinable con evidencia actual.

#### Medicion (dimension independiente de coverage)

Cada trimestre debe reportar, para el universo del scope:

    filer_continuity_securities_pct
      = securities con canonical_security matched en ambos periodos
        / securities con canonical_security en al menos un periodo

    filer_continuity_weighted_pct
      = SSHPRNAMT matched / SSHPRNAMT union

    filer_discontinuity_bruto
      = sum(abs(delta_shares)) de filas NEW + EXIT

    filer_discontinuity_neto
      = sum(delta_shares) de filas NEW + EXIT

    filer_discontinuity_concentration
      = top-5 filers por abs(delta) / total abs(delta)

Las 4 metricas se reportan siempre juntas. Thresholds para estas
metricas NO se fijan en esta version.

#### Prohibiciones explicitas

  - NO modificar C2 para "compensar" reorg de filers.
  - NO usar economic_owner_cik ni sinominos para reconciliar NT <-> HR.
  - NO reducir filer continuity a un unico ratio antes de medirla.
  - NO usar filer continuity como filtro de publicacion mientras sus
    thresholds esten UNDEFINED.
  - NO inferir continuidad economica por parent/child sin dictamen
    explicito.
...
### 14.2. Gates pendientes
    ...
    Gate-NIPC.2  Implementacion
        ...
        - Parser SEC 13(f) en identity/sec13f_list.py
        - Cache OpenFIGI
        - Tabla data/mappings/cusip_equivalence.csv (nueva)
        - Tests 12.1 a 12.6 (18 familias)
        |
        v
...
Fin de la especificacion v1.3. Fecha: 2026-09-19. HEAD local al redactar:
0d4a425. Estado: C1 + C4 aplicadas; seccion 3.15 filer_continuity
anadida. Pendiente: mini-probe Q-PROBE-5, coverage evidence, thresholds.
Gate-NIPC.2 BLOQUEADO. Las versiones v1.0, v1.1 y v1.2 se preservan como
INSTITUTIONAL_ACCUMULATION_NIPC_ESPECIFICACION_v1.0.md, _v1.1.md y
_v1.2.md respectivamente.
